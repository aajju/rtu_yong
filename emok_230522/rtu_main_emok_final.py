...
INTERVAL_TIME = 60*60*2  #초 * 분 * 시  = 2시간
#INTERVAL_TIME = 60*3*1  #초 * 분 * 시  = 3minute
...
def restart_code():
    ...
    subprocess.call(["python", "rtu_main_emok_final.py"])
...
# 결과를 하나의 문자열로 만드는 함수
def format_output(chnum_list, detnum_list, status_list, volt_list=None):
    output = ""
    if volt_list is not None:
        ...
        try:
            ...
            while True:
                ...
                if ret == "" :
                    ...
                    if (local_count == 9 and g_slack_err_count <4):
                        g_slack_err_count += 1
                        payload = {
                            "attachments": [
                                {
                                    "color": "#FF00FF",  # 색상을 지정합니다. 여기서는 보라색으로 지정합니다.
                                    "title": "장비 문제 - 이목지구 시공용",
                                    "text": "Not response : "+ str(g_slack_err_count),
                                }
                            ]
                        }
                        send_slack(payload)
                ...
                if (check_integrity(ret) == False):
                    ...
                    if (g_slack_err_count_integrity < 7):
...
while True:
    ...
    if( (g_cur_time - g_prev_time) > INTERVAL_TIME ):  
        print ("Reset Timer")

        volts = [max(sublist) for sublist in zip(*g_volt_val)]
        
        text = "전압\n"
        for i, volt in enumerate(volts, start=1):
            text += "장비 {}: {}\n".format(i, volt)
    ...
    if g_xml_send == 1:
        ...
        for i in range(0, MAX_CH):  
            ...
            result_without_volt = format_output(chnum_list, detnum_list, status_list)

            
            if g_prev_slack_without_volt[i] == result_without_volt or g_prev_slack_without_volt[i] == "x":
                g_prev_slack[i] = result
                g_prev_slack_without_volt[i] = result_without_volt
                continue
            else:
