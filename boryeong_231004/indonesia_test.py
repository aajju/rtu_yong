...
def restart_code():
    ...
    subprocess.call(["python2", "boryeong11.py"])
...
while True:
    ...
    if count >= TimeInterval:
        ...
        # 수원삼성 서버로 보내기(2det, 4ch) - id : suwon01
        xml_result_suwon = xml_text_header
        xml_result_suwon += SUWON_XML
        
        try :
            xml_result_suwon += xml_text_tail
            ret = requests.post(url, data = xml_result_suwon, headers=headers, verify=False, timeout=3)
            ret.raise_for_status()
                    ...
                    "attachments": [
                        {
                            ...
                            "title": "이벤트 변경 - 보령 율암교",  # 장비별 타이틀 다르게
