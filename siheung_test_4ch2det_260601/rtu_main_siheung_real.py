...
def restart_code():
    ...
    subprocess.call(["python2", "rtu_main_siheung_real.py"])
...
def send_request(xml_data, server_id):
    xml_result_fake = ""
    xml_result_fake = xml_text_header + xml_data
    ...
    try:
        ...
        print("$" * 10)
        print(f"Http Error ({server_id}):", errh)
        print("$" * 10)
    except requests.exceptions.ConnectionError as errc:
        print("$" * 10)
        print(f"Error Connecting ({server_id}):", errc)
        print("$" * 10)
    except requests.exceptions.Timeout as errt:
        print("$" * 10)
        print(f"Timeout Error ({server_id}):", errt)
        print("$" * 10)
    except requests.exceptions.RequestException as err:
        print("$" * 10)
        print(f"Oops: Something Else ({server_id}):", err)
        print("$" * 10)
