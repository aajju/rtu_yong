...
# 2023.6.1
# BECS, 2 Det, 4 channel
# always OK

# Detector Value
siteId_string = "move02"
#siteId_string = "bioptech01"
status_string = "1"
TimeInterval = 10 # 얼마인지 체크 필요
...
BORYEONG_XML11 ="<detector>" + \
"<siteId>boryeong11</siteId>" + \
"<chNum>1</chNum>" + \
"<detNum>1</detNum>" + \
"<status>1</status>" + \
"<distance>0</distance>" + \
...
BORYEONG_XML13 ="<detector>" + \
"<siteId>boryeong13</siteId>" + \
"<chNum>1</chNum>" + \
"<detNum>1</detNum>" + \
"<status>1</status>" + \
"<distance>0</distance>" + \
...
def restart_code():
    ...
    subprocess.call(["python2", "rtu_main_indonesia.py"])
...
### 
def send_request(xml_data, server_id):
    xml_result_fake = ""
    xml_result_fake += xml_text_header 
    xml_result_fake += xml_data
    
    try:
        xml_result_fake += xml_text_tail
        ret = requests.post(url, data=xml_result_fake, headers=headers, verify=False, timeout=3)
        ret.raise_for_status()

    except requests.exceptions.HTTPError as errh:
        print "$" * 10
        print "Http Error (%s):" % server_id, errh
        print "$" * 10
    except requests.exceptions.ConnectionError as errc:
        print "$" * 10
        print "Error Connecting (%s):" % server_id, errc
        print "$" * 10
    except requests.exceptions.Timeout as errt:
        print "$" * 10
        print "Timeout Error (%s):" % server_id, errt
        print "$" * 10
    except requests.exceptions.RequestException as err:
        print "$" * 10
        print "Oops: Something Else (%s):" % server_id, err
        print "$" * 10
...
while True:
    ...
    if count >= TimeInterval:
        ...
        try :
            ...
            ret = requests.post(url, data = xml_result, headers=headers, verify=False, timeout=3)
            ret.raise_for_status()
        except requests.exceptions.HTTPError as errh:
            print "$$$$$$$$$$"
        ...
        send_request(SUWON_XML, "suwon01")
        send_request(BORYEONG_XML12, "boryeong12")
        send_request(BORYEONG_XML14, "boryeong14")
        send_request(BORYEONG_XML13, "boryeong13")
        send_request(BORYEONG_XML11, "boryeong11")
        send_request(ILSAN_XML, "ilsan01")
        send_request(SONGPA_XML, "songpa02")
                    ...
                    "attachments": [
                        {
                            ...
                            "title": "이벤트 변경 - 시흥 160m 장비",  # 장비별 타이틀 다르게
