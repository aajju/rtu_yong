...
status_string = "1"
#TimeInterval = 60 * 60 * 2 # 얼마인지 체크 필요
TimeInterval = 300
...
def restart_code():
    ...
    subprocess.call(["python2", "rtu_main_siheung_allright.py"])
...
def clear_status_val(ch_num):
    ...
    for i in range(0, MAX_DET):  # max 7 detector per channel
        g_status_val[ch_num][i] = "1"  # 언제나 status 1 !!!  비정상 없음!!
...
def clear_xml_text():
    for i in range(0, MAX_CH):
        for j in range(0, MAX_DET):
            make_xml_text(str(i+1), str(j+1), "0", "0", "0", "0", "1")
                    ...
                    "attachments": [
                        {
                            ...
                            "title": "이벤트 변경 - siheung800 장비",  # 장비별 타이틀 다르게
